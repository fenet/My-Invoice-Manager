...
from crud import seed_company, list_clients, create_client, create_invoice_with_items, get_invoice, list_invoices, get_company, get_client
from settings import INVOICE_CITY
...
def render(tpl_name: str, **ctx):
    tpl = env.get_template(tpl_name)
    return tpl.render(**ctx)

@app.get("/", response_class=HTMLResponse)
def index(request: Request, db: Session = Depends(get_db)):
    invs = list_invoices(db)
    clients = list_clients(db)
    return render("index.html", request=request, invoices=invs, clients=clients)
...
def create_client_post(
    ...
    cl = create_client(db, company_name=company_name, address=address, uid=uid)
...
def new_invoice(request: Request, db: Session = Depends(get_db)):
    ...
    return render("new_invoice.html", request=request, clients=clients, company=company, today=date.today(), default_city=INVOICE_CITY)
...
def invoice_pdf(invoice_id: int, db: Session = Depends(get_db)):
    ...
    if not inv:
        raise HTTPException(404, "Invoice not found")
    html_content = render("invoice_pdf.html", invoice=inv)
    pdf_io = io.BytesIO()
    HTML(string=html_content, base_url=".").write_pdf(pdf_io, stylesheets=[CSS(filename="static/css/invoice.css")])
    pdf_io.seek(0)
    headers = {
